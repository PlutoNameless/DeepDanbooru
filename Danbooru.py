...
class GeM(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return F.avg_pool2d(x_clamped.pow(self.p), (x.size(-2), x.size(-1))).pow(1./self.p)
...
class MultiScaleFusion(nn.Module):
    ...
    def forward(self, features: Dict[str, Tensor]) -> Tensor:
        feature_list = [features[f'scale_{i}'] for i in range(self.num_scales)]
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs: Tensor, targets: Tensor) -> Tensor:
        ...
        if isinstance(alpha, (float, int)):
            alpha = torch.tensor(alpha, device=targets.device)
        else:
...
class DeepDanbooruV3(nn.Module):
    ...
    @staticmethod
    def _get_default_config() -> argparse.Namespace:
    ...
    def get_feature_importance(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        with torch.no_grad():
            features = self.backbone(x)
            importance = {key: feat.mean(dim=(2, 3)) for key, feat in features.items()}
        return importance


# -------------------- 模型创建与辅助函数 --------------------
def create_model(num_classes: int, **kwargs) -> DeepDanbooruV3:
    config = DeepDanbooruV3._get_default_config()
...
def test_model_compatibility():
    ...
    configs = [
        ...
        {'model_name': 'efficientnet_b0', 'fusion_type': 'weighted'},
        {'model_name': 'efficientnet_b0', 'fusion_type': 'pyramid'},
    ]
...
class GradCAM:
    ...
    def _register_hooks(self):
        ...
        for name, module in self.model.named_modules():
            if name == self.target_layer:
                ...
                break
        if not layer_found:
            logger.warning(f"Layer {self.target_layer} not found for GradCAM.")
    ...
    def generate_cam(self, x: Tensor, target_class: int) -> Tensor:
        ...
        if self.gradients is None or self.activations is None:
            raise RuntimeError("Gradients/Activations not found. Check target_layer.")
...
def load_model(filepath: str, num_classes: int = None, device: str = 'cpu') -> DeepDanbooruV3:
    ...
    config = checkpoint.get('model_config', DeepDanbooruV3._get_default_config())
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.test:
            ...
            target_tensor = torch.randint(0, 2, (batch_size, args.num_classes)).float()
